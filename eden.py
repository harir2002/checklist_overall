...
if 'structure_analysis' not in st.session_state:
    st.session_state.structure_analysis = pd.DataFrame()
...
def AnalyzeStatusManually(email=None, password=None):
    ...
    st.session_state.structure_analysis, structure_total = process_data(structure_data, structure_activity, structure_locations, "Structure")

    st.write("### Eden Structure Quality Analysis (Completed Activities):")
    st.write("**Full Output (Structure):**")
    structure_output = process_manually(st.session_state.structure_analysis, structure_total, "Structure")
    if structure_output:
            ...
            for category, activities in categories.items():
                ...
                if activities:  # For categories with activities
                    for activity in activities:
                        ...
                        closed_checklist = 0
                        if st.session_state.structure_analysis is not None and not st.session_state.structure_analysis.empty:
                            for asite_act in asite_activities:
                                matching_rows = st.session_state.structure_analysis[
                                    (st.session_state.structure_analysis['tower_name'] == tower_key) &
                                    (st.session_state.structure_analysis['activityName'] == asite_act)
                                ]
