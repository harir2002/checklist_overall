            ...
            try:
                if file.startswith("Eligo") and "Structure Work Tracker" in file:
                    ...
                    if st.session_state.ignore_month and st.session_state.ignore_year:
                        st.session_state.slabreport = ProcessGandH(io.BytesIO(response['Body'].read()), st.session_state.ignore_year, st.session_state.ignore_month)
                    else:
            ...
            for file in files:
                try:
                    if file.startswith("Eligo") and "Structure Work Tracker" in file:
                        ...
                        response = cos_client.get_object(Bucket="projectreportnew", Key=file)
                        # st.session_state.slabreport = ProcessEligo(io.BytesIO(response['Body'].read()))
                        if st.session_state.ignore_month and st.session_state.ignore_year:
                            st.session_state.slabreport = ProcessGandH(io.BytesIO(response['Body'].read()), st.session_state.ignore_year, st.session_state.ignore_month)              
    ...
    try:
        ...
        categories = {
            "COS": {
                ...
                "Interior Finishing": [
                    "Installation of doors", "Waterproofing Works", "Wall Tile", "Floor Tile"
                ],
                "ED Civil": [
...
def generate_consolidated_Checklist_excel(ai_data):
    try:
        ...
        for key in list(normalized_cos_data.keys()):
            ...
            if activity == "Concreting" and tower in slab_data_dict:
        ...
        for tower_name, slab_count in slab_data_dict.items():
            ...
            for key in asite_data_dict.keys():
                tower, activity, category = key
                if tower == tower_name and activity == "Concreting":
                    existing_key = key
                    break
            ...
            # If not found anywhere, add it to Structure Works as default
                ...
                new_key = (tower_name, "Concreting", "Structure Works")
                if new_key not in normalized_cos_data:
                    normalized_cos_data[new_key] = {"count": slab_count, "open_missing": None}
                    logger.info(f"Created new Concreting entry for {tower_name} in Structure Works: {slab_count}")
        ...
        for tower, tower_group in grouped_by_tower:
            ...
            for category, cat_group in grouped_by_category:
                ...
                for _, row in cat_group.iterrows():
                    ...
                    worksheet.cell(row=current_row, column=9).value = row["Closed checklist against completed work"]
                    worksheet.cell(row=current_row, column=10).value = row["Open/Missing check list"]
                    for col in range(6, 11):  # Columns F to I
                        cell = worksheet.cell(row=current_row, column=col)
                        cell.border = border
                        cell.alignment = center_alignment
                    current_row += 1
